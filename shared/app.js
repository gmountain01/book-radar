...
const CATS=[
  ...
  ['인공지능 입문',['인공지능 입문','처음 만나는 인공지능','난생처음 인공지능','인공지능 스타트','인공지능 첫걸음','인공지능 abc','체험 인공지능','인공지능 시대의','인공지능 1 ','인공지능 2 ','인공지능은 왜','인공지능이 답할','인공지능과 ']],
  ['파이썬',['파이썬','python']],
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (sb && !sb.classList.contains('collapsed')) {
    ...
    document.documentElement.style.setProperty('--sb-w-full', '220px');
  }
});
...
async function handleBestData(data, fname){
  ...
  let startRow = 1; // 데이터 시작 행 (보통 1: 헤더 다음)
  if(tc<0||pc<0){
    console.warn('[handleBestData] 헤더 매칭 실패, 내용 기반 자동 감지 시도. 헤더:', hdr);
    const KNOWN_PUBS = ['한빛미디어','길벗','위키북스','에이콘','인사이트','제이펍','골든래빗','영진닷컴','비제이퍼블릭','프리렉','리얼타임','생능','이지스퍼블리싱','혜지원','로드북','디코딩','더 퍼블릭','사회평론','알에이치코리아','코리아닷컴'];
    // 샘플 행(최대 20행)으로 각 컬럼 특성 분석
    const sampleRows = data.slice(0, Math.min(data.length, 21));
    const numCols = Math.max(...sampleRows.map(r => r.length));
    // 출판사: 알려진 출판사명 최다 매칭 컬럼
    let bestPubCol=-1, bestPubCount=0;
    for(let col=0; col<numCols; col++){
      const vals = sampleRows.map(r => String(r[col]||''));
      const avgLen = vals.reduce((s,v)=>s+v.length,0)/vals.length;
      // 긴 텍스트(설명) 컬럼은 출판사 후보에서 제외
      if(avgLen>50) continue;
      const pubCount = vals.filter(v => KNOWN_PUBS.some(p => v.includes(p))).length;
      if(pubCount>bestPubCount){bestPubCount=pubCount;bestPubCol=col;}
    }
    if(bestPubCount>=2) pc=bestPubCol;
    for(let col=0; col<numCols; col++){
      if(col===pc) continue;
      const vals = sampleRows.map(r => String(r[col]||''));
      const avgLen = vals.reduce((s,v)=>s+v.length,0)/vals.length;
      // 순위: 짧은 숫자(1~999)가 대부분
      if(rc<0 && avgLen<4 && vals.filter(v => /^\d{1,3}$/.test(v.trim())).length >= vals.length*0.5){
        rc=col; continue;
      }
      // 가격/SP: "원" 포함 또는 큰 숫자+쉼표 패턴
      if(sc<0 && vals.filter(v => /[\d,]+원/.test(v)).length >= vals.length*0.5){
        sc=col; continue;
      }
    }
    // 제목: 출판사/순위/가격/설명이 아닌 컬럼 중 평균 길이 10~100자로 가장 긴 것
    if(tc<0){
      let maxLen=0;
      for(let col=0;col<numCols;col++){
        if(col===rc||col===pc||col===sc) continue;
        const vals=sampleRows.map(r=>String(r[col]||''));
        const avgLen=vals.reduce((s,v)=>s+v.length,0)/vals.length;
        // 설명(수백자) 컬럼 제외, 너무 짧은(ISBN, ID) 컬럼도 제외
        if(avgLen>100 || avgLen<5) continue;
...
async function _fetchAppsScript(url) {
  ...
  if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object') {
    ...
    const dataRows = data.map(item => headers.map(h => (item[h] == null ? '' : String(item[h]))));
    // 키(headers)가 실제 컬럼명인지 데이터값인지 판별:
    // 알려진 컬럼 패턴이 하나라도 있으면 진짜 헤더로 취급
    const HEADER_PATTERNS = ['순번','순위','상품명','제목','도서','출판사','세일즈','포인트','출판일','발행일','출간일','카테고리','강의명','서비스','인기도','수강생'];
    const lowerHeaders = headers.map(h => String(h).toLowerCase());
    const hasRealHeader = HEADER_PATTERNS.some(p => lowerHeaders.some(h => h.includes(p)));
    if (hasRealHeader) {
      return [headers, ...dataRows];
    }
    // 키가 데이터값인 경우 → 키 자체를 첫 데이터 행으로 포함
    console.warn('[_fetchAppsScript] 키가 헤더가 아닌 데이터값 — 첫 행을 데이터로 포함');
    return [headers.map(String), ...dataRows];
  }
...
function renderCharts(){
  ...
  const centerLabelPlugin={
    ...
    afterDraw(chart){
      ...
      c.font="700 1.5rem 'JetBrains Mono',monospace";
      c.fillStyle='#1a1a18';
          ...
          return `<tr style="border-bottom:1px solid var(--border);">
            ...
            ${shownMonths.map(m=>{
              ...
              const cntDisplay=excludePlanning
                ?(pub.length>0?`<div style="font-family:'JetBrains Mono',monospace;font-size:.82rem;${cntColor}">${pub.length}권</div>`:'')
                :(books.length>0
                  ?`<div style="font-family:'JetBrains Mono',monospace;font-size:.82rem;${cntColor}">${books.length}권</div>`
            ...
            <td style="padding:.45rem .7rem;text-align:center;vertical-align:middle;border-left:1px solid var(--border);">
              <div style="font-family:'JetBrains Mono',monospace;font-weight:700;font-size:.85rem;">${displayTotal}</div>
