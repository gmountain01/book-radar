...
// 조사 인접 오타 (타이핑 실수) — 조사중복이 아닌 오탈자 분류
const PARTICLE_PATTERNS = [
  [/(?<=[가-힣])(?:을를|를을)(?![가-힣])/g,       '오탈자', 'high',   '을/를 중 하나만 사용 (받침 있으면 "을", 없으면 "를")'],
  [/(?<=[가-힣])(?:이가|가이)(?![가-힣])/g,       '오탈자', 'high',   '이/가 중 하나만 사용 (받침 있으면 "이", 없으면 "가")'],
  [/(?<=[가-힣])(?:은는|는은)(?![가-힣])/g,       '오탈자', 'high',   '은/는 중 하나만 사용 (받침 있으면 "은", 없으면 "는")'],
  [/(?<=[가-힣])(?:와과|과와)(?![가-힣])/g,       '오탈자', 'high',   '와/과 중 하나만 사용 (받침 있으면 "과", 없으면 "와")'],
...
const JSTYLE_PATS = [
  ...
  // "~에서의"는 단독 사용 시 자연스러울 수 있으므로 번역체 패턴에서 제외
  // → 다중조사 중첩(MULTI_PARTICLE_RE)에서 한 문장 2회 이상일 때만 조사중복으로 보고
...
//     한국어_다중조사_자료집.txt 기반 (임동훈 2004 분류 체계)
...
const UNITY_PATS = [
  ...
  [/[가-힣](?:할|된|한|인|일)때/g,  '띄어쓰기', 'high', '~할 때 (의존명사 "때" 띄어 씀)'],
  [/[가-힣](?:할|된|한|일)뿐/g,     '띄어쓰기', 'high', '~할 뿐 (의존명사 "뿐" 띄어 씀)'],
  [/[가-힣](?:할|한|된)듯/g,        '띄어쓰기', 'medium', '~한 듯 (의존명사 "듯" 띄어 씀)'],
  ...
  [/몇\s*일/g,            '맞춤법', 'high',   '며칠 ("몇 일"은 비표준)'],
  [/덮히다/g,             '맞춤법', 'high',   '덮이다'],
  [/갈려고/g,             '맞춤법', 'high',   '가려고 ("-려고"가 맞음, ㄹ 덧붙이지 않음)'],
  [/버틸려고/g,           '맞춤법', 'high',   '버티려고'],
  [/치루다/g,             '맞춤법', 'medium', '치르다'],
  [/흉칙/g,               '맞춤법', 'medium', '흉측'],
  [/몰아부치/g,           '맞춤법', 'medium', '몰아붙이다'],
  [/희안/g,               '맞춤법', 'medium', '희한'],
  [/바램/g,               '맞춤법', 'high',   '바람 ("바라다"의 명사형)'],
  [/만듬/g,               '맞춤법', 'high',   '만듦 (ㄹ받침 명사형은 -ㄻ)'],
  [/있슴/g,               '맞춤법', 'high',   '있음 (명사형 어미 "-음")'],
  [/없슴/g,               '맞춤법', 'high',   '없음'],
  [/어떻게\s*됬/g,        '맞춤법', 'high',   '어떻게 됐 ("되었"의 준말은 "됐")'],
  [/안됀다/g,             '맞춤법', 'high',   '안 된다 ("되"+"ㄴ다"="된다")'],
  [/안되요/g,             '맞춤법', 'high',   '안 돼요 ("되어요"의 준말은 "돼요")'],
  [/되요\b/g,             '맞춤법', 'high',   '돼요 ("되어요"의 준말은 "돼요")'],
  [/할께/g,               '맞춤법', 'high',   '할게 ("-ㄹ게"가 맞음)'],
  [/할꺼/g,               '맞춤법', 'high',   '할 거 ("-ㄹ 것"의 구어형)'],
  [/머릿속/g,             '맞춤법', 'low',    '머릿속 (사이시옷 — 이 표기가 맞음, "머리속" X)'],
  [/머리속/g,             '맞춤법', 'medium', '머릿속 (사이시옷 필요)'],
  [/윗부분/g,             '맞춤법', 'low',    '윗부분 (사이시옷 — 이 표기가 맞음, "위부분" X)'],
  [/아랫부분/g,           '맞춤법', 'low',    '아랫부분 (사이시옷 — 이 표기가 맞음)'],
  [/햇갈리/g,             '맞춤법', 'high',   '헷갈리 (햇 X → 헷)'],
  [/헛갈리/g,             '맞춤법', 'high',   '헷갈리 (헛 X → 헷)'],
  [/금새/g,               '맞춤법', 'high',   '금세 ("금시에"의 준말)'],
  [/일일히/g,             '맞춤법', 'high',   '일일이 ("-이"가 맞음)'],
  [/깨끗히/g,             '맞춤법', 'high',   '깨끗이 ("-이"가 맞음)'],
  [/어의없/g,             '맞춤법', 'high',   '어이없 (어의 X → 어이)'],
  [/어이없/g,             '맞춤법', 'low',    '어이없 (이 표기가 맞음)'],
  [/설레임/g,             '맞춤법', 'medium', '설렘 ("설레다"의 명사형)'],
  [/오랫만/g,             '맞춤법', 'high',   '오랜만 (오랫 X → 오랜)'],
  [/오랫동안/g,           '맞춤법', 'low',    '오랫동안 (이 표기가 맞음)'],
  [/왠지/g,               '맞춤법', 'low',    '왠지 (이 표기가 맞음, "웬지" X). 다만 "왜인지"의 줄임'],
  [/웬지/g,               '맞춤법', 'high',   '왠지 ("왜인지"의 줄임, "웬" X)'],
  [/됄/g,                 '맞춤법', 'high',   '될 ("되"+"ㄹ"="될")'],
  // ── 혼동어·자주 틀리는 표현 (한국어 교정 실무 빈출) ──
  // 되/돼 혼동
  [/되요(?=\s|[,.]|$)/g,  '맞춤법', 'high',   '돼요 ("되어요"의 준말)'],
  [/되서(?=\s|[,.]|$)/g,  '맞춤법', 'high',   '돼서 ("되어서"의 준말)'],
  [/되가(?=\s|[,.]|$)/g,  '맞춤법', 'high',   '돼가 ("되어가"의 준말)'],
  [/안되(?=\s|[,.]|$)/g,  '맞춤법', 'medium', '안 돼 ("안 되어"의 준말, 띄어쓰기+돼)'],
  // 맞춤법 혼동어 — ㄹ 탈락·불규칙
  [/울려퍼지/g,           '맞춤법', 'medium', '울려 퍼지 (울리다+퍼지다, 띄어 씀)'],
  [/갈려면/g,             '맞춤법', 'high',   '가려면 ("-려면"이 맞음)'],
  [/만들려고/g,           '맞춤법', 'high',   '만들려고 → 만들려고 (ㄹ받침 뒤 -려고 맞음, 다만 "만들어야"와 혼동 주의)'],
  [/열려고/g,             '맞춤법', 'low',    '열려고 (이 표기가 맞음)'],
  // 어미 혼동
  [/으므로써/g,           '맞춤법', 'high',   '"으므로"(이유) + "써" 불필요. "으므로" 또는 "으로써"(수단) 중 택1'],
  [/로서의/g,             '맞춤법', 'low',    '"로서"(자격) vs "로써"(수단) 구분 확인'],
  [/몇일/g,               '맞춤법', 'high',   '며칠 ("몇+일"의 본래 형태는 "며칠")'],
  // 사이시옷
  [/횟수/g,               '맞춤법', 'low',    '횟수 (이 표기가 맞음, "회수"는 "되찾음" 뜻)'],
  [/회수(?=를|가|는|의|에)/g, '맞춤법', 'low', '"회수"(되찾음) vs "횟수"(번의 수) 확인'],
  [/갯수/g,               '맞춤법', 'high',   '개수 (한자어+한자어에 사이시옷 없음)'],
  [/숫자(?!를|가|는|의|에|와|로|보다|\s)/g, '맞춤법', 'low', '숫자 (이 표기가 맞음, 사이시옷 O)'],
  [/세계관/g,             '맞춤법', 'low',    '세계관 (이 표기가 맞음)'],
  // ㅎ 탈락·불규칙
  [/파랗네/g,             '맞춤법', 'high',   '파라네 (ㅎ불규칙: 파랗+네→파라네)'],
  [/빨갛네/g,             '맞춤법', 'high',   '빨가네 (ㅎ불규칙: 빨갛+네→빨가네)'],
  [/하얗네/g,             '맞춤법', 'high',   '하야네 (ㅎ불규칙: 하얗+네→하야네)'],
  // 기타 빈출 혼동어
  [/왠만하/g,             '맞춤법', 'high',   '웬만하 ("왠"은 "왜인지"의 줄임에만 사용)'],
  [/왠만큼/g,             '맞춤법', 'high',   '웬만큼'],
  [/낳다(?=\s|[,.]|$)/g,  '맞춤법', 'low',   '"낳다"(출산) vs "낫다"(우월/회복) vs "났다"(나다의 과거) 확인'],
  [/나을수/g,             '맞춤법', 'medium', '나을 수 ("낫다"의 활용+의존명사 띄어 씀)'],
  [/예기치/g,             '맞춤법', 'high',   '예기치 → 예기(豫期)의 "예기치 못한"은 맞음. "얘기"(이야기)와 혼동 주의'],
  [/얼마만큼/g,           '맞춤법', 'medium', '얼마만큼 → 얼마만큼 (이 표기가 맞음)'],
  [/어떻해/g,             '맞춤법', 'high',   '어떡해 ("어떻게 해"의 준말은 "어떡해")'],
  [/어떻게해/g,           '맞춤법', 'medium', '어떡해 또는 "어떻게 해" (띄어 씀)'],
  [/그러므로써/g,         '맞춤법', 'high',   '"그러므로"(이유)와 "그럼으로써"(수단) 구별'],
  [/빈번히/g,             '맞춤법', 'medium', '빈번히 (이 표기가 맞음)'],
  [/솔직희/g,             '맞춤법', 'high',   '솔직히 ("솔직하다"→"솔직히")'],
  [/정확희/g,             '맞춤법', 'high',   '정확히 ("정확하다"→"정확히")'],
  [/의외로\s/g,           '맞춤법', 'low',    '의외로 (이 표기가 맞음)'],
  [/어의가\s*없/g,        '맞춤법', 'high',   '어이가 없 ("어이"가 맞음, "어의"는 의사를 뜻하는 한자어)'],
  [/뒤바뀜/g,             '맞춤법', 'medium', '뒤바뀜 (이 표기가 맞음, "뒤바뀜"="뒤바뀌다"의 명사형)'],
  [/새삼스러/g,           '맞춤법', 'high',   '새삼스러 (이 표기가 맞음, "세삼"은 비표준)'],
  [/세삼스러/g,           '맞춤법', 'high',   '새삼스러 ("세삼" X → "새삼")'],
  [/덧붙히/g,             '맞춤법', 'high',   '덧붙이 ("덧붙이다"가 맞음)'],
  [/어거지/g,             '맞춤법', 'high',   '억지 ("어거지" X → "억지")'],
  [/우겨넣/g,             '맞춤법', 'medium', '욱여넣 ("욱여넣다"가 맞음)'],
  [/담궈/g,               '맞춤법', 'high',   '담가 ("담그다"의 활용은 "담가")'],
  [/담궈놓/g,             '맞춤법', 'high',   '담가 놓 ("담그다"→"담가"+"놓다")'],
  [/곰곰히/g,             '맞춤법', 'high',   '곰곰이 ("-이"가 맞음)'],
  [/오랫만에/g,           '맞춤법', 'high',   '오랜만에 → 오랜만에 또는 "오래간만에"'],
  [/요새/g,               '맞춤법', 'low',    '요새 (이 표기가 맞음, "요즘"과 동의)'],
  [/일부로(?=\s|[,.]|$)/g, '맞춤법', 'medium', '"일부로"(일부분으로) vs "일부러"(고의로) 확인'],
  // 통일안 O/X — 띄어쓰기 핵심 (합성어는 붙여 씀)
  [/문제\s없/g,           '띄어쓰기', 'medium', '문제없 (합성어 붙여 씀)'],
  [/상관\s없/g,           '띄어쓰기', 'medium', '상관없 (합성어 붙여 씀)'],
  [/궁금해\s하/g,         '띄어쓰기', 'medium', '궁금해하 ("-어하다" 형용사 뒤 붙여 씀)'],
  [/걱정\s되/g,           '띄어쓰기', 'low',    '걱정되 (합성어 붙여 씀)'],
  [/짜증\s나/g,           '띄어쓰기', 'low',    '짜증나 (합성어 붙여 씀)'],
];
...
const _NAVER_MAX_CHUNKS = 8;   // 분당 rate limit 10회 → 여유 두고 8청크
...
async function _checkNaverSpeller(extracted) {
  ...
  for (var si = 0; si < validPages.length && sampledPages.length < _NAVER_MAX_CHUNKS; si += step) {
    sampledPages.push(validPages[si]);
  }
  // 청크 분할
  var chunks = [];
...
const CHUNK_KEYWORDS = {
  ...
  '맞춤법': ['되다', '돼', '돼야', '됩니다', '로서', '로써', '데', '든지', '던지', '다달이', '짭짤', '영락없이', '바라다', '얼마예요', '부스스', '맞추다', '맞히다', '들르다', '다르다', '틀리다', '붙이다', '부치다', '거치다', '걷히다', '시키다', '식히다', '느리다', '늘이다', '늘리다', '조리다', '졸이다', '주리다', '줄이다', '이따가', '있다가', '띠다', '띄다', '그중', '이중'],
...
- 예: found="프로젝트에서의 팀원들과의 협업에서의 소통 방식에 대한 논의가 필요하다" → suggestion="프로젝트에서 팀원들과 협업하며 소통하는 방식을 논의해야 한다"
- 예: found="사용자로부터의 피드백에 대한 분석을 통해서의 개선점을 찾아야 한다" → suggestion="사용자 피드백을 분석해 개선점을 찾아야 한다"
- 예: found="시스템에서의 데이터베이스로의 접근에서의 권한 설정에 대해서 알아본다" → suggestion="시스템에서 데이터베이스에 접근할 때의 권한 설정을 알아본다"

교정 원칙 (한국어 다중조사 자료집 기반):
1. '-에서의/에의/로의/에게의' 계열은 삭제하거나 동사로 풀기
2. '-로서의', '-로서는'은 90% 생략 가능
3. '-에 대한/에 관한/에 대해서'는 문장당 1회 이하
4. 명사화된 술어('~이 가능하다')를 동사('~할 수 있다')로 바꾸면 앞쪽 조사 중첩 해소됨

- found: 해당 문장 전체를 복사 (절대 잘라내지 말 것)
- suggestion: 다중조사를 삭제/축약하고 동사 중심 문장으로 재구성한 완성된 수정 문장
...
- 맞춤법: IT 기술서에서 자주 틀리는 맞춤법 오류를 적극적으로 찾을 것. 아래 목록은 반드시 검사해야 하는 핵심 혼동어:

  [어미·어형 혼동 — 하나라도 보이면 반드시 잡을 것]
  · 되/돼: '돼'는 '되어'의 준말. "안 돼요"○ "안 되요"× / "됐다"○ "됬다"×
  · -던/-든: '-던'은 과거 회상("했던"), '-든'은 선택("배든 사과든")
  · -데/-대: '-데'는 직접 경험("예쁘데"), '-대'는 전달("예쁘대")
  · -로서/-로써: '-로서'는 자격("편집자로서"), '-로써'는 수단("노력으로써")
  · 다르다/틀리다: '같지 않다'→다르다, '맞지 않다'→틀리다
  · -ㄹ려고: "갈려고"× → "가려고"○ (ㄹ 덧붙이지 않음)
  · 어떡해/어떻게: "어떡해"는 감탄("어떻게 해"의 준말), "어떻게"는 방법
  · 안/않: "안 가다"(부사) / "가지 않다"(용언)
  · -할게/-할께: "줄게"○ "줄께"× / "할게요"○ "할께요"×
  · 체/채: "못 본 체했다"(체하다) / "켠 채로 잠들었다"(채)
  · 있음/없음: "있음"○ "있슴"× (명사형 어미 '-음')
  · 바람/바램: "우리의 바람"○ "우리의 바램"× ('바라다'의 명사형)
  · 만듦/만듬: "만듦"○ "만듬"× (ㄹ받침 명사형은 -ㄻ)
  · 며칠/몇일: "며칠"○ "몇 일"×
  · 깨끗이/깨끗히: ㅅ받침 뒤 → '-이' (깨끗이○, 느긋이○)
  · -히: '-하다' 어근이면 '-히' (급히○, 솔직히○)
  · 삼가/삼가해: "삼가 주십시오"○ "삼가해 주십시오"× ('삼가하다'는 없는 말)
  · 가졌다/갖었다: "가졌다"○ "갖었다"× ('갖다'는 자음어미 앞에서만)
  · 붙이다/부치다: "우표를 붙이다"(접착) / "편지를 부치다"(전달) / "힘이 부치다"(부족)
  · 거치다/걷히다: "서울을 거치다"(경유) / "구름이 걷히다"(벗겨짐)
  · 시키다/식히다: "일을 시키다"(하게 하다) / "국을 식히다"(온도 낮춤)
  · 느리다/늘이다/늘리다: "느리다"(속도) / "늘이다"(길이) / "늘리다"(수량)
  · 조리다/졸이다: "생선을 조리다"(음식) / "마음을 졸이다"(초조)
  · 주리다/줄이다: "배를 주리다"(굶다) / "비용을 줄이다"(감소)
  · (정답을) 맞히다: "맞히다"○ "맞추다"× (적중 의미)
  · (색을) 띠다: "띠다"○ "띄다"× (색을 나타내다)
  · 이따가/있다가: "이따가"(조금 뒤) / "있다가"(상태 전환)
  · 따라 하기: "따라 하기"○ "따라하기"× (본동사+본동사 띄어 씀)
  · 그중/이 중: "그중"○(합성어) / "이 중"○(합성어 아님, 띄어 씀)
  · -든가/-던가: "생긴다든가"○(선택) / "생긴다던가"×(과거 회상은 -던)

  [두음법칙]
  · 단어 첫머리: 녀→여, 리→이, 렬→열, 률→율(모음·ㄴ뒤)
  · 단어 내부는 본음 유지: 남녀, 협력, 전력
  · "회계 연도"○ "회계 년도"× / "출석률"○ "출석율"×

  [사이시옷]
  · 한자어+한자어는 원칙적으로 사이시옷 없음 (초점○, 촛점×)
  · 예외 6가지: 곳간, 셋방, 숫자, 찻간, 툇간, 횟수
  · 고유어 포함 합성어: 나뭇잎, 아랫집, 전셋집

  [불규칙 활용]
  · ㅂ불규칙: 가까워(가깝-), 아름다워(아름답-)
  · ㄷ불규칙: 걸어(걷-), 들어(듣-)
  · ㅅ불규칙: 지어(짓-), 나아(낫-)
  · ㅎ불규칙: 노란(노랗-), 파란(파랗-)

- 띄어쓰기: IT 기술서에서 자주 틀리는 띄어쓰기 오류:

  [의존명사 — 반드시 띄어 씀]
  · 것/수/만큼/대로/뿐/지/데/듯/바/줄/리/터/셈/나름
  · "할 수 있다"○ "할수있다"× / "알 뿐이다"○ "알뿐이다"×
  · "하는 데 필요한"○ "하는데 필요한"× (단, 접속어미 '-는데'는 붙여 씀)

  [보조용언 — 원칙 띄어 씀, 붙여 씀 허용]
  · "먹어 보다"○ "먹어보다"△ / "알려 주다"○ "알려주다"△
  · 합성어 사전 등재: "살펴보다, 알아보다, 들여다보다"는 붙여 씀

  [접미사/조사 — 붙여 씀]
  · '-간'(시간 경과): "수년간"○ "수년 간"× / '-순': "크기순으로"○ / '-별': "제조사별로"○
  · '-ㄹ수록'(어미): "할수록"○ "할 수록"×
  · 합성어: "문제없다"○ "문제 없다"× / "필요시"○ / "밤하늘"○ / "마음속"○ / "점심시간"○

  [보조용언 핵심 구분 (다산북스 기준)]
  · '있다/듯하다/만하다/법하다/척하다' 앞: 반드시 띄어 씀 → "서 있다"○ "서있다"× / "쓸 만하다"○
  · '-어하다' 형용사 뒤: 붙여 씀 → "궁금해하다"○ "궁금해 하다"×
  · '-지 말다': 띄어 씀 → "하지 마라"○ "하지마라"×
  · 본동사+본동사: 띄어 씀 → "따라 하다"○ "따라하다"× / "붙여 넣다"○

  [자주 틀리는 IT 용어 띄어쓰기]
  · "로그인하다"○(한 단어) / "로그 인"× / "데이터베이스"○(한 단어)
  · "오픈 소스"○(두 단어) / "오픈소스"△ / "클라우드 서비스"○
  · 전문 용어 두 단어 이상: "명령 행"○ "명령행"× / "작업 환경"○ / "절대 참조"○
  · 만(萬) 단위 띄어 씀: "1만 5000"○ "15000"△
  · 영문 단위 붙여 씀: "1kg"○ "1 kg"× / "50px"○ / "300mm"○

- 문장부호오류: 2014년 문장 부호 해설 기준:
  (1) 제목·표어에 마침표 쓰지 않음
  (2) 직접 인용문 끝 마침표 원칙적 사용("떠나자."라고)
  (3) 접속부사(그러나, 하지만, 그러므로) 뒤 쉼표 원칙적 미사용
  (4) 줄임표 앞 쉼표 금지(대전……○, 대전,……×)
  (5) 겹낫표『 』=책 제목, 홑낫표「 」=소제목·법률명
  (6) 큰따옴표=직접 인용, 작은따옴표=마음속 말·강조
  (7) 연월일 '일' 뒤 마침표 필수(2019. 3. 1.)
  (8) 물결표~는 범위 표시(앞뒤 붙여 씀)
  (9) 줄임표는 여섯 점 원칙, 세 점도 허용
...
▶ 맞춤법·띄어쓰기:
  ...
  - suggestion: 올바른 표기만 제시. 규칙 설명은 description에 넣을 것.
  - 예: found="됬다" → suggestion="됐다", description="'되었다'의 준말은 '됐다'"
  - 예: found="할수있다" → suggestion="할 수 있다", description="의존명사 '수'는 띄어 씀"
  - 예: found="갈려고" → suggestion="가려고", description="'-려고'가 맞음, ㄹ 덧붙이지 않음"
...
function getCtx(text, found, ctxWindow) {
  ...
  const cw = ctxWindow != null ? ctxWindow
           : fLen > 60 ? 15   // 긴 found (문장 치환형): 앞뒤 15자만
           : fLen > 30 ? 30   // 중간 found: 앞뒤 30자
           : 50;              // 짧은 found (단어/구 단위): 앞뒤 50자
  const bs = Math.max(0, idx - cw);
  const ae = Math.min(text.length, idx + fLen + cw);
  return {
    before: (bs > 0 ? '…' : '') + text.slice(bs, idx),
    target: text.slice(idx, idx + fLen),
    after:  text.slice(idx + fLen, ae) + (ae < text.length ? '…' : '')
  };
}
...
const EDIT_CATEGORIES = [
  ...
  { key:'오탈자',      label:'오탈자·표기',     sub:'띄어쓰기·맞춤법·문장부호·공백',
    types:['띄어쓰기','맞춤법','문장부호오류','불필요한공백','오탈자'] },
...
function _cleanSuggestion(iss) {
  ...
  if (unity) return unity[1];

  // '~ 하나 삭제 또는 다른 표현' → 교체 불가, 스킵
  if (/삭제\s*(또는|혹은)/.test(s)) return '';
  ...
  const parenClean = s.match(/^([^(（]+?)\s*[(（]/);
  if (parenClean && parenClean[1].length < 20) return parenClean[1].trim();

  // 표면검사: 'X (Y → Z)' 없이 단순 교체어만인 경우
  // 예: '애플리케이션', '됐', '며칠' 등 — 짧은 단어 그대로 반환
  if (s.length <= 10 && !/[.!?,;]/.test(s)) return s;

  // AI검사 결과 정제
  let cleaned = s;

  // [오류] 접두어 제거
  cleaned = cleaned.replace(/^\[오류\]\s*/i, '');

  // 내용보완필요·사실오류: 여러 문장 중 첫 교체 문장만 추출
  // found와 비슷한 길이의 첫 문장이 교체문, 나머지는 설명
  if (iss.type === '내용보완필요' || iss.type === '사실오류') {
    // 마침표로 문장 분리 후, found 길이의 3배 이내인 부분만 취함
    const foundLen = (iss.found || '').length;
    const maxLen = Math.max(foundLen * 3, 100);
    if (cleaned.length > maxLen) {
      // 첫 마침표·첫 줄바꿈까지만
      const cut = cleaned.match(/^(.{20,}?[.다])\s/);
      if (cut) cleaned = cut[1];
      else cleaned = cleaned.slice(0, maxLen);
    }
  }
...
/** 텍스트에 교정 적용 (mark=true 시 【】마커로 교정 부분 표시) */
function _applyAll(text, corrs, mark) {
  let r = text;
  for (const c of corrs) {
    const repl = mark ? '【' + c.repl + '】' : c.repl;
    r = r.split(c.found).join(repl);
  }
  return r;
}
...
async function p8_downloadCorrected() {
  ...
  try {
    if (ext === 'txt' || ext === 'md') {
      ...
      _dlBlob(new Blob(['\uFEFF' + _applyAll(text, corrs, true)], { type: 'text/plain;charset=utf-8' }), base + '_교정.' + ext);
      ...
      for (const c of corrs) {
        ...
        // 빨간색 run으로 감싸서 교정 부분 시각 구분
        const redRun = '<w:r><w:rPr><w:color w:val="FF0000"/></w:rPr><w:t xml:space="preserve">' + er + '</w:t></w:r>';
        // XML에서 found 텍스트가 단일 <w:t> 안에 있을 수도, 여러 run에 나뉠 수도 있음
        // 단일 <w:t> 내 치환: 텍스트 레벨에서 교체 후 빨간 run 삽입
        xml = xml.split(ef).join('</w:t></w:r>' + redRun + '<w:r><w:rPr/><w:t xml:space="preserve">');
      }
      // 빈 run 정리: <w:r><w:rPr/><w:t xml:space="preserve"></w:t></w:r> 제거
      xml = xml.replace(/<w:r><w:rPr\/><w:t xml:space="preserve"><\/w:t><\/w:r>/g, '');
      ...
      for (const fn of secs) {
        ...
        for (const c of corrs) {
          // HWPX: 빨간색 charPr로 교정 부분 표시
          const redChar = '<hp:run><hp:charPr><hp:color hp:val="FF0000"/></hp:charPr><hp:t>' + c.repl + '</hp:t></hp:run>';
          xml = xml.split(c.found).join('</hp:t></hp:run>' + redChar + '<hp:run><hp:charPr/><hp:t>');
        }
        // 빈 run 정리
        xml = xml.replace(/<hp:run><hp:charPr\/><hp:t><\/hp:t><\/hp:run>/g, '');
        zip.file(fn, xml);
      }
