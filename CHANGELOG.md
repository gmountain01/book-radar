...
## 2026-09-13 — v2.7.26 (QA 수정 + Kiwi 정리)

미커밋 상태였던 v2.7.21~25 작업 전반을 QA한 뒤 발견 사항을 수정.

- **[H] CI 스모크 복구:** 단발 "결론적으로"를 `AI투` 서피스 탐지로 기대하던 픽스처가 v2.7.21 정책(단발 연결어 보호→AI 위임)과 어긋나 테스트가 29/30으로 실패. 픽스처를 정상 문장(`expectType:null`)으로 재분류하고 탐지 기준선 30→29로 조정. 이제 탐지 29/29·오탐 0/31 통과.
- **[M] 신간 저자 자동 갱신:** `fetch_aladin_new.py`가 어느 워크플로에서도 실행되지 않아 `data/aladin/new_books.json`이 9/2 스냅샷에 고정되던 문제. `fetch-rss.yml`의 build_authors 직전에 알라딘 수집 단계 추가(`continue-on-error`). `fetch_aladin_new.py`의 `sys.stdout` None 가드 보강.
- **[L] panel23:** RSS 미로드 상태에서 검색 시 "RSS 데이터 없음" 안내를 "검색 결과 없음"으로 덮어쓰던 문제 — `filterAndRender`에 미로드 가드 복원.
- **[L] panel8:** init `catch`가 `P8Review` 미정의 시 재예외를 던지던 footgun을 리터럴 폴백으로 방어. 다른 원고 선택 시 `_ignoredOnce`(이번 항목 제외) 승계를 막도록 `setFile`에서 파일 키 변경 시 초기화.
- **Kiwi 정리:** 도입 보류한 로컬 형태소 엔진(Kiwi)의 잔재 제거 — 죽은 QA 스크립트(`qa_review_browser.cjs`), `_workspace` tgz·probe, `__pycache__` 캐시 삭제, `.venv`에서 kiwipiepy·모델 언인스톨(197→69MB). 실코드(panel8·index.html)에는 원래 미반영. `test_proofread_review.js`·CHANGELOG의 Kiwi 참조 정리.
- **QA·검증:** Node 회귀 스위트 전체 통과, `node -c`·`py_compile`·YAML 파싱 통과. 브라우저 실사용 검증은 미수행. 자산 ?v=261, 화면 버전 v2.7.26.

---

## 2026-09-11 — v2.7.25 (교정 결과 목록 표시 성능 개선)

- 필터 처리에서 허용·제외 판단을 항목별 한 번만 수행하고 집계·카드·목록이 결과를 공유. 항목마다 `allIssues.indexOf`를 호출하던 작업을 인덱스 Map 조회로 변경.
- 카테고리 및 목록 HTML이 같으면 DOM 교체를 생략. 해결 상태 변경 시 목록 HTML 캐시 무효화. 화면 밖 카드의 레이아웃·그리기는 `content-visibility`로 지연하며 전체 항목과 건수는 유지.
- QA: 동일 Chromium에서 110건 캐시 중 1건 제외한 109건에 동일 필터 20회 적용하는 합산 스크립트 시간 188.5ms → 11.4ms. 단일 로컬 비교이며 첫 검사·AI 응답·모든 필터 전환의 속도 개선 수치를 의미하지 않음. 캐시 복원 110개/제외 후 109개, DOM 재사용, 해결 상태 유지, 마지막 카드 접근, 카드·목록 건수 및 회귀 테스트 통과. GitHub push는 수행하지 않음.

---

## 2026-09-11 — v2.7.24 (캐시 복원 시 빈 교정 목록 수정)

- 이전 수정은 카테고리 클릭 경로만 초기화해, 캐시 결과 진입 시 `수정 권장 0건` 필터와 이전 심각도 필터가 남을 수 있었음. 새 검사와 캐시 복원 완료 시 검토 분류·심각도·유형·검색·해결 상태 필터를 전체로 초기화. 첫 화면 및 새 파일의 기본 분류도 전체 제안으로 변경.
- 사용자가 직접 선택한 필터에 결과가 없으면 남아 있는 제안 수와 `필터 해제 · 전체 N건 보기` 버튼 표시. 허용·제외한 항목은 계속 제외.
- 실제 Chromium에서 저장소 캐시 110건을 심어 캐시 복원 경로 확인. 오래된 검색·심각도·수정 권장 필터를 남긴 상태에서도 110개 목록 렌더링. 1건 제외 후 첨부 화면과 같은 수정 권장 0 / 문맥 확인 93 / 윤문 16 / 제외 1을 재현, 전체 109개 표시 확인. 수동 필터로 0건이 된 뒤 전체 보기로 109개 복구 확인. 기존 검사 결과 캐시는 삭제하지 않음. 배포 자산 버전 259로 상향, GitHub push는 수행하지 않음.

---

## 2026-09-11 — v2.7.23 (조사 오탐 및 카드·목록 건수 수정)

- `차이가`, `나이가`, `고양이가`, `종이가`, `어린이가`, `사과와`, `결과와` 등의 명사 일부를 조사 중복으로 잡던 정규식 오탐 수정. 모호한 `이가/가이/과와`는 검토한 명사 어간 목록에 한정하며, 그 밖의 어휘는 문맥 검사에 맡김. `학생이가`, `책을를` 등 명확한 중복은 유지.
- 조사 수정안에 앞 명사 글자가 중복 삽입되던 문제 수정 (`책을를`의 `을를` → `을`). AI에도 정상 명사+조사를 구분하도록 명시. 이전 오탐 결과를 다시 쓰지 않도록 교정 캐시 버전 상향.
- 카테고리 카드 클릭 시 검토 분류·심각도·해결 상태·검색 필터를 초기화하여 카드의 전체 제안이 목록에 표시되도록 수정. 허용·제외한 항목은 카드 집계에서도 제외하고 즉시 갱신.
- QA: 기존 60개 교정 픽스처 및 검색·시장 피드 회귀 통과. 정상 어휘 13개, 실제 중복 유지, 수정안 치환, 카드 필터 및 허용 제외 건수 테스트 추가. 실제 Chromium 원고 업로드→카드 클릭→항목 제외에서 카드·목록 건수 일치 및 JS 예외 없음 확인. GitHub push는 수행하지 않음.

---

## 2026-09-11 — v2.7.22 (네이버 키 갱신)

- **네이버 키:** 2026-09-11 09:33 KST 갱신. 고정 테스트 문장으로 실서비스 정상 응답을 확인한 뒤 저장. 갱신 스크립트의 Windows 출력 인코딩 오류 수정, 검증 실패 시 기존 파일 보존 및 실패 코드 반환. RSS 작업은 네이버 실패와 관계없이 계속 진행. 키 로딩 캐시 버전 갱신.
- **QA:** 기존 교정 픽스처 60건 및 검색·시장 피드 회귀 통과. 네이버 키 검증/실패 보존 단위 테스트 3건 통과.
- **Kiwi 웹 보류:** 브라우저 WASM Kiwi 교정은 도입하지 않기로 결정, 관련 자산·스크립트 미반영. panel8은 표면/AI/네이버 검사만 유지.

---

## 2026-09-10 — v2.7.21 (교정 오탐 완화 + 공개 라이브러리 적용 + QA)

- **[H] 결과 분류:** 수정 권장 / 문맥 확인 필요 / 선택적 윤문을 심각도와 별도로 표시. 명시적인 일부 오타만 기본 수정 권장으로 표시하고 나머지는 분류별 건수로 안내. 인용·코드 내부 및 원문 위치 불확실 항목은 문맥 확인으로 처리.
- **[H] 오탐 규칙 정리:** 시제·인칭 빈도 경고 삭제, 종결어미 검사에서 인용·코드·표·캡션 제외. '하지 마라' 정상 표기를 잡던 패턴 수정. 알아보다·해내다, 허용되는 보조 용언 표기, 숫자+단위, 문맥이 필요한 어미를 일괄 오류로 잡던 패턴 제거.
- **[H] AI 지침:** 적극적으로 경계 사례까지 보고하라는 지시를 근거 중심·최소 수정으로 변경. 상세 교정규칙 18절도 함께 개정하고 생성 JS 갱신. build_교정규칙.js의 잘못된 입출력 디렉터리 수정.
- **[H] 오탐 관리:** 이번 항목 제외 / 동일 원고 허용 / 브라우저 공통 허용, 전문용어 사전, 허용 취소 제공. 파일명·크기·수정일로 원고 범위 구분. 제외는 채택과 분리하며 교정 원고 반영·보고서에서 제외. 제외한 동일 문구를 다른 위치의 전역 치환이 덮어쓰지 않도록 방어.
- **[M] es-hangul 2.4.0:** MIT 원본·라이선스·메타데이터와 브라우저 래퍼 보관, 받침 판별에 적용. 미로드 시 기존 함수 폴백.
- **[M] 캐시·출처:** 표면/AI/네이버 출처 구분, 규칙 ID와 위치 보존. 검사 캐시 pf_v2로 갱신, 기존 캐시와 혼용 방지. panel8 JS·CSS·규칙·픽스처 및 신규 자산 ?v=256. 화면 버전 v2.7.21.
- **QA:** 교정 픽스처 40→60건(오류 30건 탐지 유지, 정상 30건 오탐 0), 리뷰 정책·허용 범위·저장소 손상·제외 항목 원고 반영 방지 회귀 테스트 추가. 브라우저에서 TXT 업로드, 허용·취소, 사전 저장/복원, 보고서 다운로드, DOCX/PDF 추출을 확인. 외부 AI/네이버 요청은 QA에서 차단.
- **유지보수:** Node 기존 회귀 검사에 신규 검사 연결. es-hangul 공개 라이브러리 적용 범위는 브라우저 받침 판별. 로컬 형태소 엔진(Kiwi)은 도입 보류.

---

## 2026-09-10 — 시장 분석 피드 렌더링 최적화

- **[H] 기사 카드 페이지 분할:** 누적 기사 전체를 한 번에 렌더링하던 동작을 50건 단위 이전/다음 페이지로 변경. 현재 로컬 데이터에서 초기 카드 수 6,603개 → 50개. 전체 검색 결과 건수와 현재 표시 구간 제공.
- **[M] 검색 계산 절감:** 데이터 병합 시 날짜 정규화, 검색 문자열 가공, 전체 날짜 정렬을 한 번 수행해 색인으로 보관. 검색·소스·유형·기간 필터는 정렬된 색인을 필터링하며 전체 결과를 다시 정렬하지 않음.
- **[M] 입력 처리:** 150ms 디바운스와 한글 조합 중 검색 보류 적용. 필터 변경 시 첫 페이지로 복귀하며 페이지별 기획 보드 추가는 전체 결과의 올바른 기사 인덱스 사용.
- **대상:** panels/panel23/panel23.js·css, index.html(?v=255), scripts/test_market_feed.js, scripts/run_tests.js.
- **검증:** 실제 로컬 RSS 데이터로 페이지당 카드 수, 마지막 페이지, 기획 보드 인덱스, 첫 페이지 밖 기사 검색, 한글 조합, 빈 결과와 검색 해제 검증. 기존 회귀 테스트도 통과. 실제 브라우저 로딩 시간은 미측정이며 RSS 원본 데이터 다운로드·파싱 용량은 이번 변경에 포함하지 않음.

---

## 2026-09-10 — 메뉴 선택 시 사이드바 즉시 접기

- **[M] 사이드바 닫힘 지연:** 자동 접힘 상태에서 메뉴를 선택하면 호버 확장을 즉시 해제. 포인터가 사이드바 밖으로 나간 뒤 다시 진입하면 호버 확장 재개. 펼쳐 고정한 상태는 유지.
- **[M] 모바일 처리 순서:** 패널 활성화 훅보다 먼저 사이드바와 오버레이를 닫도록 변경.
- **대상:** shared/app.js, shared/styles.css, index.html. app.js·styles.css 캐시 버전 ?v=254.
- **검증:** 사이드바 상태 전환 모의 테스트와 기존 회귀 테스트 실행. 실제 브라우저에서 체감 지연 시간은 별도 확인 필요.

---

## 2026-09-10 — 저자 검색 및 순위 데이터 로딩 오류 수정

### panels/panel24 + panels/panel25 + shared/router.js

- **[H] 검색 결과 0건에서 검색창·필터 유지:** 화면 전체를 빈 결과 안내로 교체하던 동작을 수정. 목록 영역에 안내를 표시해 검색어 수정과 필터 해제가 가능하도록 변경.
- **[H] 검색 입력 안정화 및 렌더링 최적화:** 입력 중 검색창을 다시 만들지 않고 목록과 필터 건수만 갱신. 150ms 디바운스와 한글 조합 중 검색 보류를 적용하고, 검색 입력마다 순위 차트를 재생성하던 작업 제거.
- **[H] 순위 데이터 로딩 실패 후 복구:** 데이터가 없을 때 빈 검색 색인을 캐시하지 않도록 변경. 로딩 실패 안내와 재시도를 제공하고, 새 검색이나 추적 닫기 이후 이전 요청 결과가 표시되지 않도록 요청 번호 확인 추가.
- **[M] 공통 아카이브 로더:** 두 패널이 shared/router.js의 loadYes24Archive를 사용하도록 통합. 진행 중 요청과 로드된 데이터를 공유해 중복 스크립트 로드 방지. 실패·30초 시간 초과 시 스크립트와 로딩 상태를 정리해 재시도 허용.
- **캐시 갱신:** index.html의 router.js, panel24.js, panel25.js 참조를 ?v=253으로 변경. 화면 표기 버전은 기존 v2.7.20 유지.
- **회귀 테스트:** scripts/test_author_search.js 추가 및 scripts/run_tests.js에 연결. 검색창 유지, 한글 조합 처리, 빈 필터 결과, 동시 로딩 공유, 실패 복구, 데이터 재사용, 시간 초과 후 재시도 검증.
- **검증 결과:** 추가 회귀 테스트, 기존 JSON 처리 11건 및 교정 예문 40건 모두 통과. 변경한 JavaScript 구문 검사 통과. 브라우저 실사용 검증 및 배포는 수행하지 않음.
...
- **[H] 시장 역학 지표 3종 신설(generate_report.py `compute_insights`→`insights.js`):** 일별 순위 시계열을 평균·카운트로 뭉개던 것에서 **궤적 분석**으로. ① 순위 모멘텀(최근 30일 최소제곱 기울기 = 순위/일, 상승·하락), ② 변동성(순위 표준편차 — CV는 상위권에 불리해 절대 std 채택, "오래+좁은 밴드"로 스테디셀러 재정의), ③ 주제별 출판사 집중도 HHI(=Σ등장일점유율², 과점↔분산). 결측(200위 밖)은 값을 지어내지 않고 등장일만 사용.
- **[H] 기획 기회 종합 점수(같은 파일 `_opp_reason`+opportunity):** 3지표를 **하나로 융합** — 기회 점수 = max(모멘텀,0) × 경쟁여유(1−HHI), 시장 크기는 곱이 아니라 게이트(작은 니치 제외), 신간 진입 급등은 제외(창 시작 전부터 있던 책 추세만). 4분면 등급(🟢선점/🟡차별화/⚪관망/🔴회피) + 결론 TOP3. 실측 검증: 클라우드/DevOps·로봇/IoT가 "뜨는데 비어있음", 최대 시장 AI/LLM(1377종)은 하락→관망.
- **[H] panel23 리포트 탭 시각화:** 리포트 본문 **상단에** 「🎯 이번 주 기획 기회」결론 카드 + 「🗺 기회 맵」버블(x=모멘텀·y=경쟁여유·크기=시장·색=등급, 라벨 플러그인) 배치, 개별 3차트(모멘텀·변동성·HHI)는 「📎 세부 지표」접이식으로. 트렌드 탭에서 리포트 탭으로 이동(분석 콘텐츠 정위치). 4차트 destroy/재생성·onDeactivate 정리. Playwright로 렌더 실측 검증(산점도 x축 reverse 제거로 "좌하단=안정적 강자" 캡션 일치 수정 포함).
- **[M] 메인 사이드바 가끔 닫힘 버그 수정(styles.css):** `@media(max-width:1024px)` 태블릿 블록이 `collapsed` 여부와 무관하게 사이드바 52px+hover확장을 강제 → 펼쳐 고정해도 마우스 떼면 접힘. 규칙을 `.collapsed` 스코프로 한정해 근본 수정. 데스크톱/모바일 오프캔버스 동작 불변.
- **[M] 분석 중립화(탈한빛):** 리포트가 특정 출판사를 기준·주인공으로 서술하던 것 제거. AI 프롬프트에 "분석 관점" 규칙 추가(특정사 시점·'○○ 선두이나 추격' 서사 금지, 출판사는 데이터로만 중립 언급), 폴백 인사이트 #3을 "○○이 1위"→"출판사 집중도 상위 3사 N%"(구조가 주어)로 전환. 점유율표 등 객관적 사실은 유지. MY_PUBLISHER 미설정이라 자사 분석 섹션은 원래 없음(확인). 리포트+reports.js 재생성.
- **panel23 js·css ?v=251, styles.css ?v=251, insights.js 태그 신설, 헤더 v2.7.19.**
...
- **효과:** 내 PC 전원과 무관하게 **매일 09:00(KST) 클라우드 자동 수집→분석**. 수동 다운로드/업로드 폐지. 검증: 미국 IP(GitHub Actions)에서도 정상 다운로드(로컬과 바이트 동일 114,485), 전체 파이프라인 실행 성공(archive 243일 46,982건, "날짜 구멍 없음"). **헤더/타이틀 표기 v2.7.12 · 2026-08-31로 갱신(패널 자산 무변경이라 `?v`는 유지).**
...
- **[L] 경쟁 출판사 기본 선택 변경:** 한빛미디어·한빛아카데미만 제외하고 전체 선택되던 기본값 → **전체 선택 해제**(사용자가 직접 선택). 미선택 실행은 기존 가드("경쟁사를 1개 이상 선택하세요") 유지. 미사용이 된 `DEFAULT_EXCLUDE_PUBS` 상수 삭제. **?v=246, 헤더 v2.7.11.**
